...
contract Harry is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 30000 ether;
    uint256 public constant INITIAL_OFFERING_DISTRIBUTION = 20000 ether;
    ...
    address public harryOracle;

    address private _operator;

    /**
     * @notice Constructs the HARRY ERC-20 contract.
     */
    constructor() ERC20("HARRY", "HARRY") {
        emit OperatorTransferred(address(0), _operator);
        // Mints 1 HARRY to contract creator for initial pool setup
        _mint(msg.sender, 1 ether);
    }

    function _getHarryPrice() internal view returns (uint256 _harryPrice) {
        try IOracle(harryOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Failed to fetch HARRY price from Oracle");
        }
    }

    function setHarryOracle(address _harryOracle) public onlyOperator {
        require(_harryOracle != address(0), "oracle address cannot be 0 address");
        harryOracle = _harryOracle;
    }

    /**
     * @notice Operator mints HARRY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HARRY to mint to
     ...
     */
    function distributeGenesisPoolReward(
